...
function StarRating({ rating }) {
  ...
  return (
    <div className="flex flex-col items-end gap-0.5 shrink-0">
      <div className="flex gap-0.5 text-warning">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg 
            ...
            width="10" 
            height="10" 
            fill={star <= roundedRating ? "currentColor" : "rgba(var(--muted), 0.3)"} 
            viewBox="0 0 24 24"
          >
      ...
      <span className="text-[10px] font-bold text-fg leading-none mt-0.5">
...
function MentorCard({ mentor }) {
  ...
  return (
    <div
      ...
      className={`group relative rounded-2xl border bg-surface shadow-card hover:shadow-lift hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-all duration-200 cursor-pointer overflow-hidden gpu-layer ${
        mentor.isVerified 
    ...
    >
      {/* Top accent stripe */}
      <div className={`h-1 w-full bg-gradient-to-r ${mentor.isVerified ? "from-primary via-accent to-primary" : gradient}`} />
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 min-w-0">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            {/* Avatar */}
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-border bg-surface2 text-sm font-extrabold text-fg shadow-sm">
              {initials}
            </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {mentor.isVerified && (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-success/10 border border-success/20 text-success text-xs font-semibold">
          ...
          {sessions > 0 && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border bg-primary/10 border-primary/20 text-primary">
          ...
          {mentor.year && (
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-surface2 border border-border text-muted">
        ...
        <div className="mt-4 flex items-center justify-between border-t border-border/60 pt-3">
          ...
          <span className="text-xs font-extrabold text-primary bg-primary/10 border border-primary/20 rounded-full px-3 py-0.5 shadow-xs">
            1 Credit
          </span>
        </div>
        ...
        <div className="mt-5">
          <Button
            ...
            className="w-full rounded-full font-bold text-sm py-3"
          >
            View Profile
            <ArrowRightIcon />
          </Button>
